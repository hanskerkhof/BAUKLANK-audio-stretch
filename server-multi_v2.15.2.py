...
# SSOT topology (controllerId -> channel -> encoder deviceId)
# Ensure the folder containing this script is importable so we can load
# `time_pitch_mapping.py` when running from systemd / different CWD.
SCRIPT_DIR = Path(__file__).resolve().parent
if str(SCRIPT_DIR) not in sys.path:
    sys.path.insert(0, str(SCRIPT_DIR))

try:
    from time_pitch_mapping import TIME_PITCH_TOPOLOGY  # type: ignore
    TOPOLOGY_IMPORT_ERROR: Optional[str] = None
except Exception as e:
    TIME_PITCH_TOPOLOGY = {}
    TOPOLOGY_IMPORT_ERROR = str(e)
...
#   - encoder deviceId per channel (from SSOT map: time_pitch_mapping.py)
#
...
# SSOT topology
# ------------
# time_pitch_mapping.py defines controllerId -> channel -> encoder deviceId.
# server-multi.py reads it directly and attaches the encoder deviceId to
...
def _log_startup_debug() -> None:
    ...
    # SSOT topology summary (for sanity/debugging)
    try:
        if isinstance(TIME_PITCH_TOPOLOGY, dict) and TIME_PITCH_TOPOLOGY:
            controller_ids = sorted([str(k) for k in TIME_PITCH_TOPOLOGY.keys()])
            log.info(f"🗺️ TimePitch topology loaded: {len(controller_ids)} controller(s)")
            # Print full mapping (small, human-readable). Keep it INFO (one-time).
            for ctl in controller_ids:
                ch_map = TIME_PITCH_TOPOLOGY.get(ctl, {})
                if isinstance(ch_map, dict):
                    ...
                    suffix = ", ".join(parts) if parts else "(no channels)"
                    log.info(f"🗺️   {ctl}: {suffix}")
                else:
                    log.info(f"🗺️   {ctl}: (invalid map entry)")
        else:
            if TOPOLOGY_IMPORT_ERROR:
                log.info(
                    f"🗺️ TimePitch topology loaded: (empty) — failed to import time_pitch_mapping.py ({TOPOLOGY_IMPORT_ERROR})"
                )
            else:
                log.info("🗺️ TimePitch topology loaded: (empty) — encoder deviceId will be None")
    except Exception as e:
        log.info(f"🗺️ TimePitch topology loaded: (error: {e})")
...
def current_controller_status() -> dict:
    ...
    # Attach SSOT encoder deviceId (if available)
    try:
        ...
        for ch in ENGINE_SLOTS:
            ...
            deviceId = ctl_map.get(ch) if isinstance(ctl_map, dict) else None
            channels[ch]["deviceId"] = deviceId
    except Exception:
        pass
...
async def main():
    ...
    # One-time startup diagnostics (includes topology summary at INFO level).
    _log_startup_debug()
