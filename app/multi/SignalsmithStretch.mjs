/*
  SignalsmithStretch.mjs (vendored / generated)

  What this file does (high level):
  - Bundles the Signalsmith Stretch time-stretch/pitch-shift engine compiled to WebAssembly (via Emscripten).
  - Registers an AudioWorkletProcessor ("WasmProcessor") so the DSP runs on the audio render thread.
  - Exposes an async factory (export default) which:
      1) loads the worklet module,
      2) constructs an AudioWorkletNode,
      3) provides a small RPC layer (postMessage) to call methods inside the worklet (addBuffers, schedule, configure...).

  Editing notes:
  - Large parts are Emscripten runtime glue; treat those as generated code.
  - The most relevant sections for app-level integration are:
      * registerWorkletProcessor(...) and class WasmProcessor (DSP + message handling)
      * the exported factory at the bottom (creates the AudioWorkletNode + helper methods)
*/
...
// -----------------------------------------------------------------------------
// registerWorkletProcessor() is evaluated inside the AudioWorkletGlobalScope.
// It defines the WasmProcessor which:
//  - owns the WASM instance + memory
//  - receives commands from the main thread via this.port (MessagePort)
//  - processes audio blocks in process() and writes the output audio
//
// The main thread calls this by loading the same module into audioContext.audioWorklet
// and then creating an AudioWorkletNode with the matching processor name.
// -----------------------------------------------------------------------------
function registerWorkletProcessor(Module, audioNodeKey) {
    // The audio DSP worker running on the audio render thread.
    // IMPORTANT: keep worklet code allocation-light; avoid console logging here.

    // Message handling from the main thread.
    // The main thread sends JSON-ish command objects, e.g.:
    //   {id, method: 'configure', args: [...] }
    // and expects a response {id, ok, result|error}.
    //
    // Buffers are transferred/serialized as Float32Array chunks as needed.
    // Communication is via this.port.onmessage (RPC-style messages).
    class WasmProcessor extends AudioWorkletProcessor {
        constructor(options) {
            ...
            this.audioBuffers = [];
            // list of (multi-channel) audio buffers
            this.audioBuffersStart = 0;
            // time-stamp for the first audio buffer
        ...
        // Called for every render quantum (typically 128 frames).
        // Reads from inputs[0] channels, writes to outputs[0] channels.
        // Internally it:
        //  - updates the time-map/schedule (rate, pitch, loop, etc.)
        //  - calls into WASM to process a larger block size
        //  - copies the resulting samples into the Web Audio output buffers.
        process(inputList, outputList, parameters) {
            ...
            if (!currentMapSegment.active) {
                ...
                let blockSamples = 0;
                // current write position in the temporary input buffer
                let audioBufferIndex = 0;
                let audioSamples = this.audioBuffersStart;
                // start of current audio buffer
                ...
                while (audioBufferIndex < this.audioBuffers.length && audioSamples < inputSamplesEnd) {
                    ...
                    // start index within the audio buffer
