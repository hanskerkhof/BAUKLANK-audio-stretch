...
function createEngine(audioContext, mixNode, engineId, outputIndex) {
    ...
    for (const k in controlDefaults) {
        controlValues[k] = loadFromLocalStorage(engineId, k, controlDefaults[k]);
    }
    for (const k in configDefaults) {
        configValues[k] = loadFromLocalStorage(engineId, k, configDefaults[k]);
    }

    const gain = audioContext.createGain();
    gain.gain.value = clamp(controlValues.volume, 0, 1);
    ...
    panLeftGain.connect(mixNode, 0, 0);
    panRightGain.connect(mixNode, 0, 1);
...
function showProcessing(engine, label = 'processing audio…') {
    const el = engine.ui.filename;
    if (!el) return;
    el.textContent = label;      // UI only
    engine._processing = true;
}

function hideProcessing(engine) {
    engine._processing = false;
    updateFilename(engine, engine.currentFileName || 'audio loaded');
}
...
(async function main() {
    ...
    const engines = new Map([
        ['A', createEngine(audioContext, mixNode, 'A', 0)],
        ['B', createEngine(audioContext, mixNode, 'B', 1)]
    ]);
    ...
    async function loadDefaultAudioIntoAllEngines() {
        for (const engine of engines.values()) {
            ...
            engine.currentFileName = url.split('/').pop();
    ...
    function configChanged(engine) {
        for (const k in engine.configValues) {
            setLocalStorageIfChanged(engine.id, k, engine.configValues[k]);
        }
        if (!engine.stretch) return;

        engine.stretch.configure({
            blockMs: engine.configValues.blockMs,
            overlap: engine.configValues.overlap,
            splitComputation: !!engine.configValues.splitComputation
        });
    }

    function controlsChanged(engine, scheduleAhead = false, opts = {}) {
        for (const k in engine.controlValues) {
            setLocalStorageIfChanged(engine.id, k, engine.controlValues[k]);
        }
        if (!engine.stretch) return;

        const t = audioContext.currentTime;
        engine.gain.gain.setValueAtTime(engine.controlValues.volume, t);

        const pan = clamp(engine.controlValues.pan, -1, 1);
        engine.panLeftGain.gain.setValueAtTime((1 - pan) * 0.5, t);
        engine.panRightGain.gain.setValueAtTime((1 + pan) * 0.5, t);

        engine.stretch.schedule({
            active: !!engine.controlValues.active,
            rate: engine.controlValues.rate,
            semitones: engine.controlValues.semitones,
            tonalityHz: engine.controlValues.tonalityHz,
            formantSemitones: engine.controlValues.formantSemitones,
            formantCompensation: !!engine.controlValues.formantCompensation,
            formantBaseHz: engine.controlValues.formantBaseHz,
            loopStart: engine.controlValues.loopStart,
            loopEnd: engine.controlValues.loopEnd,
            ...(opts.input != null ? {input: opts.input} : {}),
            outputTime: t + (scheduleAhead ? 0.1 : 0)
        });
    }

    // --------------------------------------------------------
    // Wire UI (unchanged logic, filename now safe)
    // --------------------------------------------------------
...
// // We implement a simple linear pan law:
// //   pan=-1 -> 100% Left, 0% Right
// //   pan= 0 -> 50% Left, 50% Right
// //   pan=+1 -> 0% Left, 100% Right
//     const panLeftGain = audioContext.createGain();
//     const panRightGain = audioContext.createGain();
//
...
//
// function showProcessing(engine, label = 'processing audio…') {
//     updateFilename(engine, label);
//     engine._processing = true;
// }
//
// function hideProcessing(engine, finalName) {
//     engine._processing = false;
//     updateFilename(engine, finalName);
// }
//
// // ------------------------------------------------------------
// // Main
...
//             hideProcessing(engine, engine.currentFileName || 'audio loaded');
//         }
//     }
//
//
...
//         }
//
//         // Pan is expected in -1..+1. If someone sends 0..100, convert to -1..+1.
//         if (key === 'pan') {
//             const n = toFiniteNumber(value, NaN);
//             if (!Number.isFinite(n)) return;
//
...
//
//                 // UI alias: volumePercent (1..100) -> volume (0..1)
//                 if (key === 'volumePercent') {
...
//             // Safari/iOS needs a user gesture; resume here.
...
//             }
//
//             // controller status: expects msg.engine
...
//     }
//
//     // Optional: if you don't run the WS server, you can ignore errors.
//     try {
//         connectWs();
//     } catch {
//     }
