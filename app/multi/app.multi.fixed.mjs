...
function createEngine(audioContext, mixNode, engineId, outputIndex) {
    const controlDefaults = {
        ...
        volume: 1,
        pan: 0, // kept for WS compatibility, but with ChannelMerger we hard-route
        active: false,
    ...
    // Hard route into the mix node (ChannelMerger) at outputIndex
    // gain output is mono/stereo depending on stretcher, but merger will take channel 0.
    gain.connect(mixNode, 0, outputIndex);
...
// Apply incoming control/config updates, scoped to engine
